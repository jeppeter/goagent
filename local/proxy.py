...
class Http(object):
    ...
    def copy_response(self, code, headers, write=None):
        ...
        logging.info("headers %s",repr(headers))
        ...
        if content_length:
            ...
            while 1:
                ...
                if count == 0:
                	...
                	logging.info("count %d data",count)
                	write(data)
        else:
            pass
        if need_return:
...
def gaeproxy_handler(sock, address, ls={'setuplock':gevent.coros.Semaphore()}):
    ...
    logging.info("address %s",repr(address))
    ...
    if method == 'CONNECT':
        ...
        port = int(port)
        logging.info("host %s:%d google_sites %s",host,port,repr(common.GOOGLE_SITES))
        if host.endswith(common.GOOGLE_SITES) and host not in common.GOOGLE_WITHGAE:
            ...
            logging.info("get keyfile %s certfile %s",repr(keyfile),repr(certfile))
            logging.info('%s:%s "%s %s:%d HTTP/1.1" - -' % (remote_addr, remote_port, method, host, port))
